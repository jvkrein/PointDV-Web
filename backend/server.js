...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs): Registrar ação
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        ['Novo usuário cadastrado via API', newUser.rows[0].id]);
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs): Registrar login
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        ['Login realizado via API', user.id]);
...
app.put('/api/users/me', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs)
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        ['Atualizou perfil via API', req.userId]);
...
app.post('/api/auth/change-password', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs)
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        ['Alterou senha via API', req.userId]);
...
app.post('/api/eventos', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs)
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        [`Criou evento via API: ${titulo}`, req.userId]);
...
app.put('/api/eventos/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Comparação frouxa (==) para evitar problemas string vs int
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs)
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        [`Editou evento via API: ${titulo}`, req.userId]);
...
app.delete('/api/eventos/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // 1. Limpa favoritos (Tabela 3)
    await pool.query('DELETE FROM favoritos WHERE evento_id = $1', [id]);
    
    // 2. Limpa comentários se houver (Tabela 5 - Comentários) - GARANTINDO INTEGRIDADE
    await pool.query('DELETE FROM comentarios WHERE evento_id = $1', [id]);
    ...
    // REGRA DE NEGÓCIO (Tabela 6 - Logs)
    await pool.query('INSERT INTO logs_sistema (acao, usuario_id) VALUES ($1, $2)', 
        [`Excluiu evento ID: ${id} via API`, req.userId]);
...
// ==================================================================
//               ROTAS EXTRAS (PARA CUMPRIR REQUISITOS)
// ==================================================================

// GET /api/categorias (Tabela 4 - Categorias)
...
// GET /api/eventos/:id/comentarios (Tabela 5 - Comentarios)
app.get('/api/eventos/:id/comentarios', async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT c.id, c.texto, c.data_criacao, u.nome as autor 
             FROM comentarios c 
             JOIN usuarios u ON c.usuario_id = u.id 
             WHERE c.evento_id = $1 ORDER BY c.data_criacao DESC`,
            [req.params.id]
        );
        res.json(result.rows);
    } catch (err) {
...
// POST /api/eventos/:id/comentarios (Tabela 5 - Comentarios)
app.post('/api/eventos/:id/comentarios', verifyToken, async (req, res) => {
    const { texto } = req.body;
    try {
        await pool.query(
            'INSERT INTO comentarios (texto, usuario_id, evento_id) VALUES ($1, $2, $3)',
            [texto, req.userId, req.params.id]
        );
...
app.listen(PORT, () => {
  console.log(`Servidor API (Parte 2) rodando na porta ${PORT}`);
});